...
export function ClientManager({ initial }: { initial?: AdminClientListItem[] }) {
  ...
  return <>
    ...
    {open && <div className="modal-wrap" role="dialog" aria-modal="true"><form className="modal" action={submit}><header><h2>Create real client</h2><button type="button" className="ghost-icon" onClick={() => setOpen(false)} aria-label="Close"><X size={18} /></button></header><div className="modal-body form-grid">
      <div className="field full"><label>Business name</label><input name="name" required placeholder="Business name" /></div>
      <div className="field"><label>Industry</label><input name="industry" required placeholder="e.g. Recruitment" /></div><div className="field"><label>Contact name</label><input name="contact_name" required /></div>
      <div className="field"><label>Email</label><input name="email" required type="email" /></div><div className="field"><label>Phone</label><input name="phone" required type="tel" /></div>
      <div className="field"><label>WhatsApp</label><input name="whatsapp" type="tel" /></div><div className="field"><label>Website</label><input name="website" required type="url" placeholder="https://example.com" /></div>
      <div className="field"><label>Location / emirate</label><input name="location" required placeholder="Dubai" /></div><div className="field"><label>Start date</label><input name="start_date" required type="date" defaultValue={new Date().toISOString().slice(0,10)} /></div>
      <div className="field"><label>Lifecycle status</label><select name="lifecycle" defaultValue="onboarding"><option value="onboarding">Onboarding</option><option value="active">Active</option><option value="paused">Paused</option><option value="needs_attention">Needs Attention</option><option value="archived">Archived</option></select></div>
      <div className="field full"><label>Business description</label><textarea name="description" required /></div>
      <fieldset className="field full"><legend>Service scope (no plans)</legend><div className="service-scope-options">{[["seo", "SEO"], ["social_media", "Social Media · 12 posts/month"], ["blogs", "Blogs · 2/month"], ["website_maintenance", "Website Maintenance"], ["website_chatbot", "Website AI"], ["whatsapp_ai", "WhatsApp AI"], ["analytics_reporting", "Analytics / Reporting"]].map(([key, label]) => <label key={key}><input type="checkbox" name={key} /> {label}</label>)}</div></fieldset>
    </div><div className="modal-actions"><button type="button" className="button secondary" onClick={() => setOpen(false)}>Cancel</button><button className="button" disabled={saving}>{saving ? "Creating…" : "Create isolated workspace"}</button></div></form></div>}
  </>;
}
