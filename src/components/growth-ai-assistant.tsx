...
export function GrowthAiAssistant({ data }: { data: ClientResultsData }) {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", text: `Hi — I can explain your verified results for ${data.periodLabel}. What would you like to know?` }]);
  ...
  async function ask(text: string) {
    ...
    try {
      const response = await fetch("/api/client/assistant", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: prompt, range: data.rangeKey, from: data.rangeStart, to: data.rangeEnd }) });
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "The assistant could not answer. Please try again."); }
    finally { setLoading(false); }
  }
  function submit(event: FormEvent) { event.preventDefault(); void ask(question); }

  return <>
    <button className="growth-ai-launcher" aria-label="Ask Growth1000 AI" aria-expanded={open} onClick={() => setOpen(value => !value)}>{open ? <X size={22}/> : <GrowthAiIcon/>}<span>Ask Growth1000 AI</span></button>
    {open && <aside className="growth-ai-panel" aria-label="Growth1000 AI assistant">
      <header><div className="growth-ai-avatar"><GrowthAiIcon size={23}/></div><div><b>Growth1000 AI</b><span><i/>Grounded in {data.periodLabel}</span></div><button aria-label="Close assistant" onClick={() => setOpen(false)}><X size={18}/></button></header>
      <div className="growth-ai-log" ref={log} aria-live="polite">
        ...
        {error && <div className="growth-ai-error"><b>I couldn’t answer that.</b><span>{error}</span><button onClick={() => void ask(messages.at(-1)?.text ?? "")}>Try again</button></div>}
      </div>
      <form onSubmit={submit}><label htmlFor="growth-ai-question">Ask about your results</label><div><input id="growth-ai-question" value={question} onChange={event => setQuestion(event.target.value)} placeholder="Are our leads improving?" maxLength={500}/><button aria-label="Send question" disabled={!question.trim() || loading}><ArrowUp size={17}/></button></div><small><Sparkles size={11}/>Uses verified dashboard data only</small></form>
    </aside>}
  </>;
}
