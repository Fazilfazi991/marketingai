...
export function ClientTrafficDetail({ data }: { data: ClientResultsData }) {
  ...
  const declined = data.search.keywords.filter(
      (i) => i.current > i.previous,
    ).length,
    stable = data.search.keywords.filter(
      (i) => i.current === i.previous,
    ).length;
  return (
    <>
      <Link className="detail-back" href="/client">
        ...
        <section className="detail-surface">
          <header>
            <div>
              ...
              <h2>{data.search.keywords.length} tracked keywords</h2>
            </div>
            <p>
              {data.search.improved} improved · {declined} declined · {stable}{" "}
              stable
            </p>
          </header>
