...
async function boundedFetch(raw:string,allowedHost:string,accept:string){
 ...
 for(let redirects=0;redirects<=3;redirects+=1){
  ...
  let response:Response;try{response=await fetch(current,{headers:{Accept:accept,"User-Agent":"Growth1000WebsiteInventory/1.0"},redirect:"manual",signal:controller.signal,cache:"no-store"})}finally{clearTimeout(timer)}
  ...
  const body=(await response.text()).slice(0,WEBSITE_INVENTORY_LIMITS.maxBytes);return{response,body,url:current};
 }
...
export async function inspectWebsite(rootRaw:string):Promise<{rootUrl:string;discovered:number;pages:InspectedPage[]}>{
 const root=await validatePublicHttpUrl(rootRaw),host=root.hostname.toLowerCase();
 const queue:string[]=[],seen=new Set<string>(),sitemaps=new Set<string>([new URL("/sitemap.xml",root).toString()]);
 try{const robots=await boundedFetch(new URL("/robots.txt",root).toString(),host,"text/plain");for(const item of robotsSitemaps(robots.body).slice(0,WEBSITE_INVENTORY_LIMITS.sitemaps))sitemaps.add(item)}catch{}
 for(const sitemap of [...sitemaps].slice(0,WEBSITE_INVENTORY_LIMITS.sitemaps)){try{const result=await boundedFetch(sitemap,host,"application/xml,text/xml");for(const item of sitemapUrls(result.body))queue.push(item)}catch{}}
 if(!queue.length)queue.push(root.toString());
 const pages:InspectedPage[]=[];
 while(queue.length&&pages.length<WEBSITE_INVENTORY_LIMITS.pages){const raw=queue.shift()!;let candidate:URL;try{candidate=await validatePublicHttpUrl(raw,host)}catch{continue}const key=candidate.toString().replace(/\/$/,"");if(seen.has(key))continue;seen.add(key);
  try{const{response,body,url}=await boundedFetch(candidate.toString(),host,"text/html,application/xhtml+xml");const contentType=response.headers.get("content-type")??"";if(!contentType.includes("text/html"))continue;const canonicalRaw=/<link\b[^>]*rel=["'][^"']*canonical[^"']*["'][^>]*href=["']([^"']+)["']/i.exec(body)?.[1]??/<link\b[^>]*href=["']([^"']+)["'][^>]*rel=["'][^"']*canonical/i.exec(body)?.[1];let canonical=url;try{if(canonicalRaw)canonical=await validatePublicHttpUrl(new URL(canonicalRaw,url).toString(),host)}catch{}
   ...
   if(pages.length<WEBSITE_INVENTORY_LIMITS.pages)for(const item of links(body,url)){try{const child=new URL(item);if(child.hostname.toLowerCase()===host&&!seen.has(child.toString().replace(/\/$/,"")))queue.push(item)}catch{}}
  }catch{}
 }
 return{rootUrl:root.toString(),discovered:seen.size+queue.length,pages};
}
