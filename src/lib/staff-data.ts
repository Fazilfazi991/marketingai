...
function formatSchedule(value: string | null) {
  if (!value)
    return { month: "Unscheduled", date: "Date pending", time: "Time pending" };
...
const demoItems: StaffQueueItem[] = posts
  .filter((post) =>
    ["Ready to schedule", "Scheduled", "Published", "Issue"].includes(
      post.status,
    ),
  )
  .map((post) => ({
    ...
    issueNote: "",
    status: post.status as StaffQueueStatus,
    color: post.color,
    storagePath: null,
    imageUrl: null,
  }));
...
export async function loadStaffQueue(): Promise<StaffQueueData> {
  ...
  if (!user) throw new Error("Sign in as a staff member.");
  ...
  const { data: links, error: linkError } = ids.length
    ? await supabase
        .from("content_assets")
        .select("content_item_id,version,assets(storage_path)")
  ...
  for (const link of links ?? []) {
    const key = String(link.content_item_id);
    if (paths.has(key)) continue;
    const relation = link.assets as unknown as
      { storage_path?: string } | { storage_path?: string }[] | null;
    const asset = Array.isArray(relation) ? relation[0] : relation;
    paths.set(key, asset?.storage_path ?? null);
  }
  const storedPaths = [...paths.values()].filter((path): path is string =>
    Boolean(path),
  );
  const signed = storedPaths.length
    ? await supabase.storage
        .from("client-assets")
        .createSignedUrls(storedPaths, 900)
    : { data: [], error: null };
  if (signed.error) throw signed.error;
  const urls = new Map(
    (signed.data ?? []).map((item) => [item.path, item.signedUrl]),
  );
  const items = (data ?? []).map((row, index): StaffQueueItem => {
    const relation = row.clients as unknown as
      { name?: string } | { name?: string }[] | null;
    const client = Array.isArray(relation) ? relation[0]?.name : relation?.name;
    const storagePath = paths.get(String(row.id)) ?? null;
    return {
      id: String(row.id),
      client: client ?? "Client",
      ...formatSchedule(
        row.published_at ?? row.scheduled_at ?? row.recommended_publish_at,
      ),
  ...
  const clients = [...new Set(items.map((item) => item.client))].sort();
  const first = (data ?? []).find(
    (row) => row.recommended_publish_at,
