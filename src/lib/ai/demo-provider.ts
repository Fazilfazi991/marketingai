...
export class DemoAIProvider implements AIProvider{
 ...
 async analyzeSEO(_context:GenerationContext,input:string){let keywords:Array<{keyword?:string;target_url?:string|null;current_position?:number|null}>=[];try{keywords=(JSON.parse(input) as {keywords?:typeof keywords}).keywords??[]}catch{}return result(keywords.slice(0,4).map(item=>item.current_position&&item.current_position<=20?`Strengthen ${item.target_url||"the target page"} for ${item.keyword||"the tracked keyword"} with clearer service relevance and verified proof.`:`Review search intent and on-page coverage for ${item.keyword||"the tracked keyword"} before expanding the target page.`))}
 async generateMonthlyReport(_context:GenerationContext,input:string){let facts:Record<string,unknown>={};try{facts=(JSON.parse(input) as {facts?:Record<string,unknown>}).facts??{}}catch{}const source=facts.largestLeadSource as {label?:string;count?:number}|null;return result(`${facts.clientName??"The client"} generated ${facts.leads??0} tracked enquiries this month, including ${facts.qualified_leads??0} qualified. Leads were ${Number(facts.lead_change)>=0?"up":"down"} ${Math.abs(Number(facts.lead_change)||0)}% from the previous month.${source?` ${source.label} was the largest source with ${source.count}.`:""} The website recorded ${facts.users??0} users and ${facts.clicks??0} organic search clicks.`)}
}
