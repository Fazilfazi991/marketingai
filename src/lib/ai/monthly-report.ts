...
export async function prepareMonthlyReport(provider: AIProvider, context: GenerationContext, clientName: string, month: string, input: ReportInputs) {
  ...
  const analyticsSummary = { leads, lead_change: percentChange(leads, input.leadsBefore.length), qualified_leads: qualifiedLeads, users, user_change: percentChange(users, previousUsers), clicks, click_change: percentChange(clicks, previousClicks), social_posts: posts, blogs };
  ...
  const generated = await provider.generateMonthlyReport(context, JSON.stringify({ instruction: "Write a concise plain-language client result summary using only these facts. Do not invent causes, percentages, claims or recommendations.", facts }));
