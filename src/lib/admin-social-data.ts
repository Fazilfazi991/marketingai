...
const imageStatus=(value:string|undefined):SocialPost["imageStatus"]=>value==="generated"?"Generated":value==="failed"?"Failed":"Placeholder";
...
export async function loadAdminSocial():Promise<AdminSocialData>{
  ...
  const{data:generations,error:generationError}=contentIds.length?await supabase.from("image_generations").select("content_item_id,model,version,status,storage_path").in("content_item_id",contentIds).order("version",{ascending:false}):{data:[],error:null};
  if(generationError)throw generationError;
  const latestGeneration=new Map<string,{model:string;version:number;status:string;storage_path:string|null}>();
  for(const item of generations??[])if(!latestGeneration.has(String(item.content_item_id)))latestGeneration.set(String(item.content_item_id),item);
